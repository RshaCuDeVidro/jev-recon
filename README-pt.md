...
> Versão em inglês: [README.md](README.md). Esta é a versão em português, com o
> mesmo conteúdo.
...
├── pyproject.toml
├── BENCHMARK.md          o benchmark, em ingles
...
[
  {
    ...
    "reasons": [
      "api 0.97  superficie de API para maquina",
      "sensitive 0.96  guarda dado ou funcao sensivel",
      "production 0.98  sistema em producao",
      "code: labels privilegiados admin, api",
...
* `signals`: as probabilidades (Noul) por pergunta, 0 a 1. `null` quando a
  resposta não veio.
* `reasons`: por que aquele asset ficou onde ficou. Calculado em Python, a partir
  das probabilidades e dos fatos, nunca gerado por modelo. A ordem é a
  contribuição de cada sinal (`peso × valor`), então a primeira linha é o motivo
  que puxou o score. Linhas de fato vêm prefixadas com `code:` ou `evidence:`.
...
### `likely_devops`, fora do default

Existe um sinal extra para infraestrutura de build e release (CI/CD, registries,
artifact stores, controladores de deploy), disparado por nomes como `jenkins`,
`ci`, `build`, `deploy`, `argocd`, `drone`, `tekton`. Ele não está no conjunto
default porque custa mais uma pergunta por asset, cerca de 12% mais tokens.
Ligue e pondere:
...
### Por que um asset ficou onde ficou

`reasons` é calculado em código, a partir dos números e dos fatos:
...
Um sinal aparece quando passa de 0.60, ordenado por `peso × valor`, então a
primeira linha é o motivo que moveu o score. Depois vêm os fatos: token de
ambiente, labels privilegiados, resposta gated (`HTTP 401/403`), título da
página, tecnologias detectadas. Asset sem nada acima do piso imprime
`no signal above the floor` em vez de um motivo inventado. No terminal sai com
`--reasons N`, no arquivo como `reasons[]`.
...
## 7. O ranking semântico ganha de uma lista de palavras?

`BENCHMARK.md`, com `scripts/make_benchmark.py` e `scripts/benchmark.py`. Cinco
métodos (aleatório, heurística de nome, regex de keywords sobre a evidência, Jev
com nome, Jev com nome e evidência), medidos em precisão e recall no top 5, 10 e
20 por cento, em 300 hosts cujos nomes não carregam informação nenhuma sobre o
rótulo, média de três sorteios de rótulo:
...
* Só nome fica no nível do aleatório, e isso é o desenho do teste: sem evidência
  não existe sinal a extrair, seja qual for o método.
* No top 10% o regex está ligeiramente à frente (ele maximiza precisão achando só
  o óbvio). No top 20% ele desaba para 0.689 enquanto o Jev segura 0.956, com
  recall 0.637 contra 0.459. A lista de palavras esgota o que tem para achar em
  torno de 30% dos alvos; o Jev continua achando.
* No tier de evidência ambígua (título genérico atrás de 401/403) o regex acerta
  zero por construção, e o Jev só chega lá depois que o `criteria` descreve essa
  condição explicitamente. O achado negativo e a seção do que o benchmark não
  prova estão em `BENCHMARK.md`.

## 8. Testes e demo sem API key
